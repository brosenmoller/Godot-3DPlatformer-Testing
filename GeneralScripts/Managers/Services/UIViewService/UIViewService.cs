...
public class UIViewService : Service
{
    ...
    private void SetupViewDictionary()
    {
        ...
        foreach (UIView view in views)
        {
            ...
            if (defaultViewFound)
            {
                GD.PrintWarning("UIViewManager: Multiple default views found in current scene");
            }
            else
            {
        ...
        if (!defaultViewFound)
        {
            GD.PrintWarning("UIViewManager: No default views found in current scene");
        }
    }
